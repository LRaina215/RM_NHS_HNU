...
cv::Mat NumberClassifier::extractNumber(const cv::cuda::GpuMat & src, const Armor & armor) const noexcept
{
  ...
  // 0122 CUDA
  // cv::cuda::warpPerspective(src, number_image, rotation_matrix, cv::Size(warp_width, warp_height));
  
  cv::cuda::GpuMat gpu_warp;
  cv::cuda::warpPerspective(src, gpu_warp, rotation_matrix, cv::Size(warp_width, warp_height));
  gpu_warp = gpu_warp(cv::Rect(cv::Point((warp_width - roi_size.width) / 2, 0), roi_size));
  ...
  gpu_warp.download(number_image);
